...
from worddetectornn.aabb import AABB
from worddetectornn.coding import encode
from worddetectornn.utils import compute_scale_down, prob_true
...
class DataLoaderImgBytes:
    """Loader for already-loaded images (as byte streams or numpy arrays)"""

    def __init__(self, images, input_size, device, max_side_len=1024):
        """
        images: list of
            - raw image bytes (e.g. b'...') OR
            - numpy arrays (already decoded images)
        """
        self.images = images
        self.input_size = input_size
        self.device = device
    ...
    def _decode(self, img):
        """Ensure image is a NumPy grayscale array"""
        if isinstance(img, bytes):
            arr = np.frombuffer(img, np.uint8)
            img = cv2.imdecode(arr, cv2.IMREAD_GRAYSCALE)
        elif isinstance(img, np.ndarray):
            if img.ndim == 3:  # convert to grayscale if color
                img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        else:
            raise TypeError("Images must be bytes or numpy arrays")
        return img

    def __getitem__(self, item):
        orig = self._decode(self.images[item])
        ...
        img = (img / 255 - 0.5).astype(np.float32)
        imgs = img[None, None, ...]   # shape: (1, 1, H, W)
        imgs = torch.from_numpy(imgs).to(self.device)
        return DataLoaderItem(imgs, None, None)

    def get_scale_factor(self, item):
        img = self._decode(self.images[item])
        f = min(self.max_side_len / img.shape[0], self.max_side_len / img.shape[1])
        return f if f < 1 else 1
